...
db = Database("sqlite", "pydataSchedule/pydataSchedule.sqlite", create_db=True)
...
class PyDataSqlite():
	...
	def getResultDB(self):
		...
		try:
			# SELECT
			countSessions =  PyDataSession.select().count()
			print '\nNumber sessions ' + str(countSessions)	    
			
			sessions = PyDataSession.select_by_sql("SELECT * FROM PyDataSession")		
		    
			lista =[];
			
			# Imprimimos el registro resultante
			for session in sessions:
				talk = session.talk.decode('utf-8')
				speaker = session.speaker
				description = session.description
				date = session.date
				
				register = (talk,speaker,description,date)
...
class PyData(Gtk.Window):
	...
	def __init__(self):
		...
		Gtk.Window.__init__(self, title="PyData conferences")
		...
		self.conferenceList = Gtk.ListStore(str,str,str,str)
		...
		for i, column_title in enumerate(["Title","Speaker","Description","Date"]):
		...
		for days in ["Friday","Saturday","Sunday"]:
	...
	def day_filter_func(self, model, iter, data):
		...
		if self.current_filter_date is None or self.current_filter_date == "None":
			...
			print self.current_filter_date
			print model[iter][3]
	...
	def on_menu_createDataBase(self, widget):
		...
		try:
			pyDataSqlite = PyDataSqlite()
        
			self.label.set_markup("<i>"+"BD created PyData"+"</i>")
	...
	def on_menu_getDataFromDataBase(self, widget):
		...
		try:
			...
			listaConferencias = pyDataSqlite.getResultDB()
	...
	def on_menu_dialog_info(self, widget):
		...
		dialog.format_secondary_text(
		        "Se conecta con el sitio hhttp://pydata.org/madrid2016/schedule/ para obtener las conferencias,las registra en la base de datos y las visualiza en la ventana\n\n"
	...
	def on_menu_lanzarScrapy(self, widget):
		...
		print("scrapy crawl pydataSpiderDetails")
		self.label.set_markup("<b>"+"Ejecutar comando >> scrapy crawl pydataSpiderDetails"+"</b>")
		os.system("scrapy crawl pydataSpiderDetails")
